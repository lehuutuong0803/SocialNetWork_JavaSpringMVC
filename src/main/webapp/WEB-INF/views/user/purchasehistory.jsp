...
<div class="gap no-gap signin whitish medium-opacity">
	...
	<div class="col-lg-6"
		style="padding: 2rem; margin-left: auto; margin-right: auto;color: #252428;">
		<div class="load-more">
			<h1 style="text-align: center; margin-bottom: 20px; color: #252428;">*Lịch
				Sử Đơn Hàng*</h1>
				...
				<div class="central-meta item"
					...
					<div class="classic-post">
						...
						<div class="classic-pst-meta" style="max-width:100%;">
							...
							<c:forEach var="itemOwner" items="${owner}">
								<c:if test="${item_invoice.owner_id == itemOwner.value.id}">
									<h4 style="font-size:20px;color:black">
										Người
										bán: <a
							...
							<!--action='<c:url value="/user/addcart/${item_invoice.id}"></c:url>'  -->
							<div style="display:inline-block;" id="area_historyinvoice${item_invoice.id}">
								<c:if test="${item_invoice.status ==  0}">
									<button style="background: #fa6342;
										    border: medium none;
										    border-radius: 30px;
										    color: #fff;
										    padding: 5px 10px;"  class="main-btn" title="">Chờ Xác nhận</button>
								</c:if>
								...
								<button style="background: #fa6342;
									...
									data-bs-target="#detailed_${item_invoice.id}" class="main-btn"
									title="">Xem chi tiết</button>
							
						</div>
					</div>
				</div>

				<div class="modal fade" id="detailed_${item_invoice.id}"
										...
										<c:if test="${item_Detail.invoice_id == item_invoice.id}">
											<c:forEach var="item_Product" items="${productdetail}">
												<c:if test="${item_Product.id == item_Detail.product_id}">
													<div style="display:flex" class="classic-post">
														<figure>
														...
														<div class="classic-pst-meta">
															<div class="more">
																<div class="more-post-optns">
																	<i class="ti-more-alt"></i>
																	<ul>
																		<li><i class="fa fa-pencil-square-o"></i>Chỉnh
																			sửa</li>
																		<li><i class="fa fa-trash-o"></i>Xóa</li>
																	</ul>
																</div>
															...
															<span style="font-size:20px;" onchange="amount()" class="prise">Giá:
																${item_Detail.formatCurrency} ₫</span>
															<div class="location-area">
																<i style="font-size:15px;color:black">Số lượng: ${item_Detail.amount}</i>
															</div>
